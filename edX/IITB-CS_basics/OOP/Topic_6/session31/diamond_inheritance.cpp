...
/*
 * Diamond inheritance example: program does not compile as we don have virtual derivation
 */
...
float totalMarks;
	public:
	total(int a, float b, float c):midsem(a,b),endsem(a,c){}
	float getTotal(){
	return midsem::getMidSemMarks()+endsem::getEndSemMarks();
	}
};

/* Diamond structure of inheritance: Actually looks like a diamond(rhombus)
 *
 *              __(midsem)__
 *             /            \
 * (student)--<   (diamond)  >--(total)
 *             \__(endsem)__/
 * 
 */
...
cout<<s1.getRollNo();//call to getRollNo() is ambiguous as there are two candidates 
